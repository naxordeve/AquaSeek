...
const serialize = async (_msg, conn) => {
    try {
        if (_msg?.key) {
            ...
            _msg.device = _msg.id.startsWith('3A') ? 'ios' 
                        : _msg.id.startsWith('3EB') ? 'web'
                        : _msg.id.length === 21 ? 'android'
                        : _msg.id.length === 18 ? 'desktop' 
                        : 'unknown';
        ...
        _msg.sendButtons = async (chat, judul, teks, button, naxor) => {
    try {
                    ...
                    interactiveMessage: proto.Message.InteractiveMessage.create({
                        contextInfo: {
                            ...
                            businessMessageForwardInfo: {
                                businessOwnerJid: jidNormalizedUser(_msg.user?.id || "")
                            }
                        },
