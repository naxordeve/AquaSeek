...
interface PinterestResponse { status: string; code: number; data: PinterestItem[]; }
interface WikipediaItem { title: string; link: string; description: string; imageUrl?: string; size: string; lastUpdated: string; }
interface WikipediaResponse { status: string; data: string; }
async function searchPinterest(query: string): Promise<PinterestItem[] | { error: string }> {
  if (!query) return { error: 'Query is required' };
  const res = await fetch(`https://itzpire.com/search/pinterest?query=${query}`);
  if (!res.ok) return { error: `${res.status}` };
  const data: PinterestResponse = await res.json();
  if (data.status === "success" && data.code === 200) return data.data.map(item => ({
