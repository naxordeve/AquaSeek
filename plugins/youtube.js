...
CreatePlug({
  ...
  execute: async (message, conn, match) => {
    ...
    const db = new YTVideo("https://api.diioffc.web.id/api/download/ytmp4?url=");
...
CreatePlug({
  ...
  execute: async (message, conn, match) => {
    ...
    const url = 'https://api.diioffc.web.id/api/search/ytplay?query=';
    const ytSearch = new YouTubePlay(url);
    const video = await ytSearch.Ytdl(match);
    await conn.sendMessage(message.user, { image: { url: video.thumbnail },caption: `*Title:* ${video.title}\n*Author:* ${video.author.name}\n*Views:* ${video.views}\n*Duration:* ${video.duration}\n*Published:* ${video.publishedAgo}\n\n*Download:* ${video.download.url}`,});
    await conn.sendMessage(message.user, {
...
CreatePlug({
  ...
  execute: async (message, conn, match) => {
    await message.react("✅");
    if (!match) return message.reply("_Provide a YouTube URL_");
    const down = new YTDown("https://api.diioffc.web.id/api/download/ytmp3?url=");
